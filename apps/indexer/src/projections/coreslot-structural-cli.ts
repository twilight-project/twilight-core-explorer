...
import {
  ...
  type CoreSlotStructuralProjectionPrisma,
} from './coreslot-structural.js';
...
async function main(): Promise<void> {
  if (!process.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required for the CoreSlot structural projection');
  }
  ...
  try {
    await withProjectionAdvisoryLock(prisma, async () => {
      ...
      const cursor = await getOrCreateProjectionCursor(prisma, CORESLOT_STRUCTURAL_PROJECTION, chainId);
      ...
      await projectCoreSlotStructuralRange({
        prisma: prisma as unknown as CoreSlotStructuralProjectionPrisma,
        chainId,
        startHeight,
        endHeight,
      });
